          ...
          <Card key={r.id}>
            <CardContent className="p-4">
              ...
              <div className="mt-3 flex flex-wrap gap-1">
                {perms.slice(0, 6).map((p) => <Badge key={p} size="sm" variant="ghost"><code>{p}</code></Badge>)}
                {perms.length > 6 && <Badge size="sm" variant="ghost">+{perms.length - 6}</Badge>}
              </div>
              <div className="mt-3 text-[10px] text-slate-400">{userCount} user{userCount === 1 ? "" : "s"} assigned</div>
            </CardContent>
          </Card>
        );})}
      </div>
