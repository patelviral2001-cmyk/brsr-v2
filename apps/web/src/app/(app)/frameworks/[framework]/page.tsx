        ...
        <div className="col-span-12 lg:col-span-4">
          <Card>
            <CardContent className="max-h-[760px] overflow-y-auto p-3 scrollbar-thin">
              {sections && <BRSRSectionTree sections={sections} activeId={activeId} onSelect={setActiveId} />}
            </CardContent>
          </Card>
        </div>
          ...
          {activeSection && (
            <>
              <div className="rounded-xl border border-slate-200 bg-gradient-to-br from-primary-50 via-white to-white p-5">
                <div className="text-xs uppercase tracking-wider text-primary-700">Principle {activeSection.principle.slice(1)}</div>
                <h2 className="mt-1 text-xl font-bold text-slate-900">{activeSection.title}</h2>
                <p className="mt-2 text-sm text-slate-600">{activeSection.answered} of {activeSection.total} questions answered.</p>
              </div>
              {activeSection.questions.map((q) => <BRSRQuestionCard key={q.id} question={q} />)}
