...
export function EnergyMixChart({ data }: { data: { source: string; mwh: number; renewable: boolean }[] }) {
  let ri = 0, ni = 0;
  const colorized = data.map((d) => ({ ...d, fill: d.renewable ? RENEW[ri++ % RENEW.length] : NON_RENEW[ni++ % NON_RENEW.length] }));

  return (
    <ResponsiveContainer width="100%" height={260}>
      <PieChart>
        <Tooltip contentStyle={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 10, fontSize: 12 }} formatter={(v: number) => `${v.toLocaleString("en-IN")} MWh`} />
